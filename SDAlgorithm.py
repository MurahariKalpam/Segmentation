...
from testpackage import Region
...
class SDAlgorithm():
    ...
    def analyze_page(self, url):
        """ Analyzes a web page given by url """
        page = open("C:\\Users\\Murahari_Kalpam\\Desktop\\new.html", "r")
        return self.analyze_page_code(page.read())

    def analyze_page_code(self, html_code):
        if self._verbose:
            print("Create DOM tree...")
        tree = self.construct_page_tree(html_code)
        node = tree.getroot()
    ...
    def construct_page_tree(self, html_code):
        """
        Creates the DOM page tree for the html code provided as parameter.
            ...
            if grouped_comments.has_key(max_group): 
                ...
                if self.big_areas_in_same_level(article, grouped_comments, max_group) and not validated:
                    if self._verbose:
                        ...
                        print ("Class: ")
                        print (grouped_comments[max_group][0].class_name)
                        print ("Texts: ") 
    ...
    def calculate_distances_from_max(self, tree, fixed_regions=False):  
        ...
        for i,region in enumerate(tmp_regions):
            ...
            if region.distance_from_max == 0 and region.parts == 1 \
                ...
                or self.close_diff_from_second_max()): #and
